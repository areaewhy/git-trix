...
•	Another version:
o	Look in the incoming/outgoing in VS, right click and say “revert”
o	“soft” will keep the changes as staged/working-area changes, “hard” will undo them
•	!Note!: using hard can delete your work! If that happens, see reflog
...
•	Stash all changes
o	git stash
o	Stash only unstaged items with git stash -k
...
o	You can open each solution in a separate VS window and work on each separately
o	Branches/commits/fetch/pulls are shared (creating a branch in one shows in the other, pulling changes to ‘dev’ will pull for both instances, etc)
o	You cannot checkout the same branch in both worktree folders
...
o	git fetch --prune
o	( do this automatically by setting git config --global fetch.prune true, then it will be done every fetch )
## What Changed?
...
o	git remote prune origin && git branch -vv | grep ': gone]' | awk '{print $1}' > branches-to-delete.txt
...
o	while read r; do git branch -D $r; done<branches-to-delete.txt
